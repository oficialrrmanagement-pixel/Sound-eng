...
function techFor(c){return cacheProfiles.find(p=>String(p.id)===String(c.technician_id))||null}
...
async function refreshData(){
 try{
  ...
  cacheConcerts=cr.data||[];cacheProfiles=pr.data||[];
  return true;
 }catch(err){console.warn('WhatsApp data refresh failed',err);return false}
}
...
function attach(card,c){
 ...
 actions.appendChild(button('Copiar mensagem',()=>copyText(messageFor(c,techFor(c),t)),'copy'));
}
async function decorate(){
 if(decorating)return;decorating=true;
 try{
  const host=q('concerts');if(!host)return;
  if(!cacheConcerts.length)await refreshData();
  const cards=[...host.querySelectorAll(':scope > .item')];
  cards.forEach((card,i)=>attach(card,cacheConcerts[i]));
 }finally{decorating=false}
}
async function detectNewAfterSubmit(){
 await refreshData();lastIds=new Set(cacheConcerts.map(c=>String(c.id)));
 setTimeout(async()=>{
...
function observeAgenda(){
 const host=q('concerts');if(!host||host.dataset.whatsappObserved)return;
 host.dataset.whatsappObserved='1';
 new MutationObserver(()=>setTimeout(async()=>{await refreshData();decorate()},80)).observe(host,{childList:true,subtree:false});
}
function wire(){
 observeAgenda();
