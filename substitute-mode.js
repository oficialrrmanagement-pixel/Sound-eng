...
function hideInvitesForSubstitute(){
  if(!isSubstitute())return;
  const inviteList=q('inviteList');
  if(inviteList){inviteList.innerHTML='';inviteList.classList.add('hidden');const heading=inviteList.previousElementSibling;if(heading&&heading.tagName==='H3')heading.classList.add('hidden')}
...
function applySubstituteMode(){
  ...
  hideInvitesForSubstitute();renderSubstituteArtists();lockFilesReadOnly();
}
...
function boot(){patchLoadAll();patchOpenArtistFiles();setTimeout(applySubstituteMode,200);setTimeout(applySubstituteMode,700);const list=q('artistList');if(list&&!list.dataset.subLockObserved){list.dataset.subLockObserved='1';new MutationObserver(()=>{if(isSubstitute())setTimeout(()=>{hideInvitesForSubstitute();renderSubstituteArtists();lockFilesReadOnly()},30)}).observe(list,{childList:true,subtree:false})}}
